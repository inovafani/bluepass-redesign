...
export type PartnerProfileSettingsResult =
  | { ok: true }
...
/**
 * A partner editing their own public-facing details — handle and social links. Deliberately not
 ...
 */
export async function updatePartnerProfileDetails(input: {
  ...
  youtubeUrl: string;
  tiktokUrl: string;
}): Promise<PartnerProfileSettingsResult> {
  const values = {
